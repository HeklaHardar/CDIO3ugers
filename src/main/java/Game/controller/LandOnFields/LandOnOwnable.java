...
public class LandOnOwnable {
    ...
    public void Ownable(MatadorGui matadorGUI, FieldController fieldProperties, Player currentPlayer, int playerID, Player[] player, int dices) {
        ...
        } else if (((OwnableField)fieldProperties.getFields()[currentPlayer.getCurrentPosition()]).getOwner() != 0 && player[((OwnableField)fieldProperties.getFields()[currentPlayer.getCurrentPosition()]).getOwner() - 1].playerString() != player[playerID].playerString()) {
